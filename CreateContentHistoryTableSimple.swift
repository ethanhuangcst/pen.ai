...
func createContentHistoryTable() {
    ...
    do {
        ...
        for row in describeResult {
            ...
            if let field = row.column("Field"), let fieldName = field.stringValue,
               let type = row.column("Type"), let typeName = type.stringValue,
               let nullable = row.column("Null"), let nullableValue = nullable.stringValue,
               let key = row.column("Key"), let keyValue = key.stringValue,
               let defaultValue = row.column("Default"), let defaultValueValue = defaultValue.stringValue,
               let extra = row.column("Extra"), let extraValue = extra.stringValue {
